...
export function ReviewsList({ reviews, enabled }: { reviews: Review[]; enabled: boolean }) {
  ...
  return <div>
    {message ? <p aria-live={message.kind === "success" ? "polite" : "assertive"} className={message.kind === "success" ? "form-success" : "form-error"} role={message.kind === "success" ? "status" : "alert"}>{message.text}</p> : null}
